...
public class SwitcherPanel extends ViewGroup {
	...
	private boolean shouldCollapsed() {
		...
		return status == Status.EXPANDED && left <= interceptX && interceptX <= right && top <= interceptY && interceptY <= bottom;
	}
	...
	private boolean shouldExpanded(@NonNull MotionEvent event) {
		...
		return status == Status.COLLAPSED && left <= event.getRawX() && event.getRawX() <= right && top <= event.getRawY() && event.getRawY() <= bottom;
	}
